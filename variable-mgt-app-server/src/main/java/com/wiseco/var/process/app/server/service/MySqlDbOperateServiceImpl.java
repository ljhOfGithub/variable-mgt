...
@Slf4j
@Service("mySqlDbOperateServiceImpl")
@CallLogCondition.OnMysqlCallLogEnabled
public class MySqlDbOperateServiceImpl implements DbOperateService {
    ...
    static {
        ...
        DATE_TYPE_MAP.put("boolean", "tinyint(1)");
        DATE_TYPE_MAP.put("long", "bigint(20) unsigned");
        DATE_TYPE_MAP.put("text", "text");
        DATE_TYPE_MAP.put("longtext", "longtext");
    }
    ...
    public void createTable(String tableName, LinkedHashMap<String, String> columMap, List<String> indexColum, String primaryKey) {
        ...
        //创建字段
        columMap.forEach((key, value) -> sql.append(key).append(" ").append(DATE_TYPE_MAP.get(value)).append(","));
        ...
        //创建索引
        indexColum.forEach(item -> sql.append("key idx_").append(item).append("(").append(item).append(") USING BTREE,"));
        sql.deleteCharAt(sql.length() - 1);
